...
model_eval_fnc <- function(the_data = NULL,
  ...
  frmls <- list(SwampTimothy =
                ...
                Watergrass =
                  list(fixed = mass_mg ~ log(length_mm) +
...
summarize_plot_fnc(rslt_list = NULL) {# Start summarize_plot function definition
...
# Checks for correctness of list, etc. TBA
  ...
  Tbl_cap <- paste("Table of cross validation results for ", spp, " when data for each refuge is kept out for final testing and not used at all in the cv. A ", mdl.opt, " model was used. Keep in mind that the Rsq does not consider deviations from the 1:1 line but from a fitted line between observed and predicted values.", sep = "")
  
    # Cross validation table
  knitr::kable(cross_val_metrics,
               digits = c(0, 3, 3, 3),
               caption = Tbl_cap) %>%
    kable_styling(full_width = FALSE)
  
  # Cross validation figure
  ggplot(data = cross_val_metrics, aes(x = rmse, y = rsq)) +
...
st_model_eval1 <- model_eval_fnc(the_data = st_d2m, mdl.opt = "fixed")

st_model_eval2 <- model_eval_fnc(the_data = st_d2m, mdl.opt = "mixed")

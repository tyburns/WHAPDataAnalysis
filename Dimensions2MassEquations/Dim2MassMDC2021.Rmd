...
sw_dim %<>% dplyr::filter(vol_mm3 != max(vol_mm3)) # to exclude oddball
...
Predictiions resulting from the model have to be backtransformed into mg by the inverse transformation (only the case for lambda not 0 shown):
...
sw_m4 <- lm(bcPower(mass_mg, lambda = sw_lamb1) ~ log(vol_mm3) +
              width_mm + length_mm,
            data = sw_dim)
...
sw_m5 <- lm(bcPower(mass_mg, lambda = sw_lamb1) ~ log(vol_mm3) + length_mm,
            data = sw_dim)
...
sw_dim %<>% mutate(bc_mass_gm = bcPower(mass_mg, lambda = sw_lamb1))
...
results <- resamples(list(sw_m4_cv = sw_m4_cv,
                          sw_m5_cv = sw_m5_cv))
...
The two models perform almost identically. Model sw_m5 is preferred because it is simpler.

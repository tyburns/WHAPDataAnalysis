...
```{r st_plotRanef, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r st_plotAllData, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r st_estimates, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r wg_modelRandomGroups, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r wg_estimates, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r sw_transformedVolume, echo=FALSE, warning=FALSE, message=FALSE}
...
sw_m7 <- lm(bcPower(mass_mg, lambda = sw_lamb3) ~ log(vol_mm3) + width_mm,
            data = sw_dim)
...
```{r sw_cv, echo=FALSE, warning=FALSE, message=FALSE}
...
Even in cross-validation both models perform almost identically. Model 6 is slightly better in most criteria, but it also includes an additional predictor. Because the additional predictor is one of the dimensions necessary to calculate volume, dropping the predictor does not reduce field work or any other cost, so we keep it.

Note that in 2021, width was not measured in all refuges, so we used a model that only requires seed head length. The script fo future years has to be modified according to what dimensions are measured in the field.

# Model Evaluation

This section applies to Swamp Timothy and Watergrass because we have measurements for these species in more than one refuge-year combination. The evaluation focuses on determining the potential increments in precision and accuracy obtained by developing a dimensions-to-mass (d2m) model for each species in each refuge and year when the protocol is applied.
...
Predictions that deviate from observed values in a linear relationship can have very large rsq (good) and also large rsme (bad). If the slope of the relationship is 1.0 the bias is constant over X. If the slope is different from 0, the bias changes as X changes.

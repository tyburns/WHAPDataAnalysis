...
```{r evalFun}
# Model option mld.opt has to be either "fixed" or "mixed"
model_eval_fnc <- function(the_data = NULL,
  ...
  frmls <- list(SwampTimothy =
                   list(fixed = mass_mg ~ length_mm,
                        mixed = mass_mg ~ length_mm +
                                          (length_mm|LIT_yr)),
                 Watergrass =
                   list(fixed = mass_mg ~ length_mm +
                                          f2t_length_mm +
                        ...
                        mixed = mass_mg ~ length_mm +
                                          f2t_length_mm +
                                          (0 + f2t_length_mm | LIT_yr) +
                                          emerged + (0 + emerged | LIT_yr)))
  ...
  for (grp in grps){ # Start loop over LIT_yr groups ******
    ...
    df_test %<>% 
      mutate(.pred = if(mdl.opt == "fixed") {
               predict(all_fit,new_data = df_test_baked)$.pred
        } else {
          predict(all_fit,
                  new_data = df_test_baked,
                  type = "raw",
...
st_model_eval <- model_eval_fnc(the_data = st_d2m, mdl.opt = "mixed")
...
anova(wg_d2m_m2, wg_d2m_m4) # wg_d2m_m4 is the best

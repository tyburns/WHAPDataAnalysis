...
act <- read.csv("WHAP2021_PP_20220922_MUs_KRN_Actions.csv",
...
colnames(qdt_mass_act_fld)[5] <- "tilled"
...
brm2 <- brm(st_g_m2 | weights(wt) ~ tilled,
            data = qdt_mass_act_fld,
            iter = 6000,
            warmup = 2500,
            thin = 2)
...
There is a clear difference between units that were disked and those that were not. The posterior median difference is `median(brm2 %>% spread_draws(b_tilledY) %>% pluck("b_tilledY"))` g/m2 with a standard error of `sd(brm2 %>% spread_draws(b_tilledY) %>% pluck("b_tilledY"))` g/m2.

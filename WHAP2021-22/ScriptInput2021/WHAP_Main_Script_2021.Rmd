---
title: "Calculation of Timothy and Watergrass seed abundance"
...
# Fix duplicate records by keeping most recent one

# cps0 %<>% # CAN'T USE THIS BECAUSE TIME STAMPS ARE ALSO DUPLICATED
#   mutate(dateTime = mdy_hm(eventDate)) %>%
#   group_by(recordedBy,
#                 LIT,
#                 unitName,
#                 subunitName,
...
qdt_st <- qdt0 %>%
  ...
  summarise(avg_length_mm = mean(measurementValue, na.rm = TRUE),
            n_seed_heads = mean(nSeedHeads, na.rm = TRUE)) %>%
...
names(qdt_wg)[names(qdt_wg) == "sh_length"] <- "length_col"
names(qdt_wg)[names(qdt_wg) == "f2t_length"] <- "F2T_col"

sh_lengths_mm_wg <- as.matrix(qdt_wg$length_col) %>% na_if(0)
sh_F2T_mm_wg <- as.matrix(qdt_wg$F2T_col) %>% na_if(0)
sh_emerged_wg <- sh_F2T_mm_wg / sh_lengths_mm_wg

wgc <- unname(coef(WG_len2mass))

sh_mass_mg_mat <- exp(wgc[1] +
                        wgc[2] * sh_lengths_mm_wg +
                        wgc[3] * sh_emerged_wg +
                        wgc[4] * sh_F2T_mm_wg) - 20
...
qdt_wg <- qdt_wg %>%
  mutate(qdt_m2 = ifelse(Quadrat_Size == "15x15", 0.15^2,
         ...
         avg_length_mm = 10 * rowMeans(sh_lengths_mm_wg, na.rm = TRUE),
...
st_rlm3 <- rlm(mass_g_m2 ~ LIT_Strat, # This has heterogeneity of variance
              na.action = na.exclude,
              maxit = 40,
             data = qdt_st)
...
wg_rlm3 <- rlm(mass_g_m2 ~ LIT_Strat, # This has heterogeneity of variance
              na.action = na.exclude,
              maxit = 40,
             data = qdt_wg)

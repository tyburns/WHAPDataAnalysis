...
SW_dim2mass <- lm(bcPower(mass_mg, lambda = 0.2671952) ~ log(length_mm),
                  data = sw_dat)
...
qdt_sw <- qdt0 %>%
  ...
  na.omit() %>%
  ungroup() %>%
  mutate(mass_g_m2 = unname((0.2671952 * predict(SW_dim2mass, newdata = .) + 1)^(1/0.2671952)) / 1000) %>%
  mutate(stratum = dplyr::recode(stratum,
                                 `low` = "a.Low",
                                 `medium` = "b.Med",
                                 `high` = "c.High"),
  ...
  tryCatch({
    ...
    result <- .p_Area %>% # create file to predict mass/area by stratum & LIT
      ...
      mutate(st_g_m2 = invBoxCox(unname(predict(.st_rlm3, newdata = .)),
                                 lambda = .st_lambda)) %>%
      mutate(LIT_Strat = factor(ifelse(oLIT_Strat %in% .wg_rlm3$xlevels[[1]],
                                       as.character(oLIT_Strat),
                                       NA))) %>%
      mutate(wg_g_m2 = invBoxCox(unname(predict(.wg_rlm3, newdata = .)),
                                 lambda = .wg_lambda)) %>%
             #LIT_Strat = oLIT_Strat, # not selected below, so it's useless
      mutate(LIT_Strat = factor(ifelse(oLIT_Strat %in% .sw_rlm3$xlevels[[1]],
                                       as.character(oLIT_Strat),
                                       NA))) %>%

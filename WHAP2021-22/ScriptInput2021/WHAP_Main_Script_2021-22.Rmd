...
st_d2m_dat <- readRDS("st_d2m_dat.rds") %>%
  rename_with(~ gsub(".", "_", .x, fixed = TRUE))

# Data to create model for WG: mass = f(seed no., seed length)
# From wg_model_creation.R
wg_d2m_dat <- readRDS("wg_d2m_dat.rds") %>%
  rename_with(~ gsub(".", "_", .x, fixed = TRUE))

# Data to create model for SW: mass = f(seed no., seed length)
sw_d2m_dat <- readRDS("sw_d2m_dat.rds") %>%
  rename_with(~ gsub(".", "_", .x, fixed = TRUE))
```

## Models to go from seed head dimensions to mass
...
closure_proportions <- function(df, .in_cols, .out_cols) {
  df <- df %>% mutate(tot_obs_prop = rowSums(across(starts_with("pVA_"))))
  for (nm in .in_cols) {
...
vps <- vps0 %>%
  ...
  pivot_wider(
    names_from = c(vernacularName, stratum),
    names_prefix = "pVA_",
...
cps <- cps0 %>%
  ...
  group_by(
    GlobalID, #         | This section to be removed
...
## Subunit sampling summary

Summary at the subunit level that shows the # of quadrats, CPs, and VPs that include each species.

```{r subunit summary}

# quadrat summary
qdt_sum <- qdt_2021 %>%
  group_by(LIT, subunit_ID, species) %>%
  summarise(n = n()) %>%
  pivot_wider(names_from = species, values_from = n) %>%
  rename(st_qdt = st, wg_qdt = wg, sw_qdt = sw)

# VP summary
vps_sum <- vps %>%
  mutate(st_pres = 0, wg_pres = 0, sw_pres = 0)

for (i in 1:nrow(vps_sum)) {
  row <- vps_sum[i, ]
  # this should be improved by accessing the columns by a standard name and not
  # with the numbers 17, 18, and 19
  vps_sum[i, 17] <- ifelse(row$p_Swamp_Timothy_a.Low != 0 ||
    row$p_Swamp_Timothy_b.Med != 0 ||
    row$p_Swamp_Timothy_c.High != 0, 1, 0)

  vps_sum[i, 18] <- ifelse(row$p_Watergrass_a.Low != 0 ||
    row$p_Watergrass_b.Med != 0 ||
    row$p_Watergrass_c.High != 0, 1, 0)

  vps_sum[i, 19] <- ifelse(row$p_Smartweed_a.Low != 0 ||
    row$p_Smartweed_b.Med != 0 ||
    row$p_Smartweed_c.High != 0, 1, 0)
}

vps_sum <- vps_sum %>%
  dplyr::select(LIT, subunit_ID, st_pres, wg_pres, sw_pres) %>%
  group_by(LIT, subunit_ID) %>%
  summarise(st_vps = sum(st_pres), wg_vps = sum(wg_pres), sw_vps = sum(sw_pres))

# CP summary
cps_sum <- cps %>%
  mutate(st_pres = 0, wg_pres = 0, sw_pres = 0)

for (i in 1:nrow(cps_sum)) {
  row <- cps_sum[i, ]

  cps_sum[i, 17] <- ifelse(row$p_Swamp_Timothy_a.Low != 0 ||
    row$p_Swamp_Timothy_b.Med != 0 ||
    row$p_Swamp_Timothy_c.High != 0, 1, 0)

  cps_sum[i, 18] <- ifelse(row$p_Watergrass_a.Low != 0 ||
    row$p_Watergrass_b.Med != 0 ||
    row$p_Watergrass_c.High != 0, 1, 0)

  cps_sum[i, 19] <- ifelse(row$p_Smartweed_a.Low != 0 ||
    row$p_Smartweed_b.Med != 0 ||
    row$p_Smartweed_c.High != 0, 1, 0)
}
...
cps_sum <- cps_sum %>%
  dplyr::select(LIT, subunit_ID, st_pres, wg_pres, sw_pres) %>%
  group_by(LIT, subunit_ID) %>%
  summarise(st_cps = sum(st_pres), wg_cps = sum(wg_pres), sw_cps = sum(sw_pres))

# complete summary
qdt_vps_sum <- full_join(qdt_sum, vps_sum)
subunit_sum <- full_join(qdt_vps_sum, cps_sum)
subunit_sum <- subunit_sum %>% mutate_all(~ replace(., is.na(.), 0))

write.csv(subunit_sum, "Subunit_Sampling_Summary2021.txt")
```
...
v_Area <- vpcp %>%
  group_by(subunit_ID) %>% # see www.analyticalgroup.com/download/WEIGHTED_MEAN.pdf
  summarise(across(
    starts_with("p_"),
...
fpcf <- vpcp %>%
  ...
  left_join(su_areas) %>%
  mutate(
    fpcf = 1 - AreaSampled_ac / area_ac, # in case area sampled > subunit area
  ...
  tryCatch(
    {
      ...
      result <- .p_Area %>% # create file to predict mass/area by stratum & LIT
        ...
        mutate(
          wg_g_m2 = rowSums(dplyr::select(., starts_with("wg")) *
...
mass_kg_LIT <- qdt_mass_g_m2 %>%
  ...
  na.omit() %>%
  summarise(
    tot_mass_kg_lit = sum(tot_g_m2 * area_ha * 10),
  ...
  mutate(
    tot_kg.ha = tot_mass_kg_lit / tot_ha,
...
The tibble qdt_bt contains original data, 4000 bootstrap samples of wg quadrats, 4000 bootstrap samples of st quadrats, and the corresponding estimates of mass_m2 in each subunit for each species and total. SW was added in 2021.
...
mass.su.CI <-
  ...
  summarize(
    wg_q10 = quantile(wg_g_m2, 0.10, na.rm = TRUE), # get percentiles etc.
...
mass.su.CI %>%
  kable(
    digits = c(0, rep(1, 9)),
    format = "html",
    caption = "Seed head mass per unit area (g/m2) and 80% CI by species in each subunit. These estimations exclude the uncertainty due to the proportion of area covered by each stratum in each unit, and the uncertainty of the model to convert seed number and seed length into mass, but they include the uncertainty due to sampling quadrats."
  ) %>%
  kable_styling(full_width = FALSE)
```
...
mass_kg_LIT_CI <- qdt_bt %>%
  ...
  mutate(
    mass_su = map(.x = mass_su, ~ drop_na(.x)),
    mass_lit = map(
      .x = mass_su,
      ~ group_by(.x, LIT) %>%
        summarise(
...
mass_kg_LIT_CI %>%
  ...
  kable(
    digits = 0,
    format = "html",
    caption = "Total (all three species) seed head mass per unit area (kg/ha) and per refuge, and 80% CI by refuge. These estimations exclude the uncertainty due to the proportion of area covered by each stratum in each unit, and the uncertainty of the model to convert seed number and seed length into mass, but they include the uncertainty due to sampling quadrats."
  ) %>%
  kable_styling(full_width = FALSE)
...
mass_kg_LIT_CI %>%
  ...
  kable(
    digits = 0,
    format = "html",
    caption = "Swamp timothy and watergrass seed head mass per unit area (kg/ha) and per refuge, and 80% CI by refuge. These estimations exclude the uncertainty due to the proportion of area covered by each stratum in each unit, and the uncertainty of the model to convert seed number and seed length into mass, but they include the uncertainty due to sampling quadrats."
  ) %>%
  kable_styling(full_width = FALSE)
...
mass_kg_LIT_CI %>%
  ...
  kable(
    digits = 0,
    format = "html",
    caption = "Swamp Timothy seed head mass per unit area (kg/ha) and per refuge, and 80% CI by refuge. These estimations exclude the uncertainty due to the proportion of area covered by each stratum in each unit, and the uncertainty of the model to convert seed number and seed length into mass, but they include the uncertainty due to sampling quadrats."
  ) %>%
  kable_styling(full_width = FALSE)
...
mass_kg_LIT_CI %>%
  ...
  kable(
    digits = 0,
    format = "html",
    caption = "Watergrass seed head mass per unit area (kg/ha) and per refuge, and 80% CI by refuge. These estimations exclude the uncertainty due to the proportion of area covered by each stratum in each unit, and the uncertainty of the model to convert seed number and seed length into mass, but they include the uncertainty due to sampling quadrats."
  ) %>%
  kable_styling(full_width = FALSE)
...
mass_kg_LIT_CI %>%
  ...
  kable(
    digits = 0,
    format = "html",
    caption = "Smartweed seed head mass per unit area (kg/ha) and per refuge, and 80% CI by refuge. These estimations exclude the uncertainty due to the proportion of area covered by each stratum in each unit, and the uncertainty of the model to convert seed number and seed length into mass, but they include the uncertainty due to sampling quadrats."
  ) %>%
  kable_styling(full_width = FALSE)
```
...
Xs2 <- v_Area_c %>%
  arrange(subunit_ID) %>%
  dplyr::select(
    p_Swamp_Timothy_a.Low, # variance of areas
...
Xm2 <- p_Area %>%
  ...
  arrange(subunit_ID) %>%
  dplyr::select(
    p_Swamp_Timothy_a.Low, # means of proportion of areas
    p_Swamp_Timothy_b.Med,
    p_Swamp_Timothy_c.High,
    p_Watergrass_a.Low,
    p_Watergrass_b.Med,
...
## Precision graph (subunit CIs vs. number of quadrats taken)

```{r precision graph}

qdt_per_species <- read.csv("qdt_summary_2021.txt") %>%
  mutate_all(~ replace(., is.na(.), 0))

CI_qdt_sum <- full_join(qdt_per_species, qdt_mass_g_m2) %>%
  mutate(
    CI_len_ST = CI80upr_st - st_g_m2,
    CI_len_WG = CI80upr_wg - wg_g_m2,
    CI_len_SW = CI80upr_sw - sw_g_m2,
    CI_len_tot = CI80upr - CI80lwr,
    tot_qdt = st_qdt + wg_qdt + sw_qdt
  ) %>%
  mutate(
    CI_prop_st = CI_len_ST / st_g_m2,
    CI_prop_wg = CI_len_WG / wg_g_m2,
    CI_prop_sw = CI_len_SW / sw_g_m2
  ) %>%
  dplyr::select(
    LIT, subunit_ID, st_qdt, CI80lwr_st, st_g_m2, CI80upr_st,
    CI_len_ST, wg_qdt, CI80lwr_wg, wg_g_m2, CI80upr_wg, CI_len_WG,
    sw_qdt, CI80lwr_sw, sw_g_m2, CI80upr_sw, CI_len_SW, tot_qdt,
    CI80lwr, tot_g_m2, CI80upr, CI_len_tot, CI_prop_st, CI_prop_wg,
    CI_prop_sw
  )

ggplot(CI_qdt_sum, aes(x = st_qdt, y = CI_prop_st)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE, fullrange = TRUE) +
  xlab("\nNumber of quadrats with swamp timothy") +
  ylab("Half CI / seed mass estimate\n")

ggplot(CI_qdt_sum, aes(x = wg_qdt, y = CI_prop_wg)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE, fullrange = TRUE) +
  xlab("\nNumber of quadrats with watergrass") +
  ylab("Half CI / seed mass estimate\n")

ggplot(CI_qdt_sum, aes(x = sw_qdt, y = CI_prop_sw)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE, fullrange = TRUE) +
  xlab("\nNumber of quadrats with smartweed") +
  ylab("Half CI / seed mass estimate\n")
```
...
for (i in 1:length(unique(qdt_mass_g_m2$LIT))) { # This is an error. See Therese's script.
  plot <- ggplot(
    data = qdt_mass_g_m2 %>% arrange(LIT, tot_g_m2),
    aes(
      x = subunit_ID,
      y = tot_g_m2,
      ymin = CI80lwr,
...
for (i in 1:length(unique(qdt_mass_g_m2$LIT))) {
  plot <- ggplot(
    ...
    xlab("subunitName") +
    ylab("Timothy seed mass 80% CI (g/m^2)") +
...
for (i in 1:length(unique(qdt_mass_g_m2$LIT))) {
  plot <- ggplot(
    ...
    xlab("subunitName") +
    ylab("Watergrass seed mass 80% CI (g/m^2)") +
...
for (i in 1:length(unique(qdt_mass_g_m2$LIT))) {
  plot <- ggplot(
    ...
    xlab("subunitName") +
    ylab("Smartweed seed mass 80% CI (g/m^2)") +
...
compare_su_mass <- qdt_mass_g_m2 %>%
  ...
  nest() %>%
  mutate(
    tot_mass = map(data, ~ compare.all.su(.x)),

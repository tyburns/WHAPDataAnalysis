---
title: "Calculation of Timothy and Watergrass seed abundance. Summer 2021"
...
# PROBLEM: there are no quadrats in PIX st high but a couple of units had vps or cps with st high.
...
**These calculations assume no overlap among species.**

The script has to be modified to accept any number of species and to accommodate the absence of some combinations of species_stratum.
...
qdt0 <- read.csv("Quadrats_Midseason_20210823.csv")
names(qdt0)[1] <- "GlobalID"
...
# Fix incorrect PIX 5 to PIX 4.
vps0$unitName[vps0$LIT == "PIX" & vps0$unitName == "5"] <- "4"
...
mus0 %<>%
  mutate(subunitName = ifelse(subunitName == "", unitName, subunitName),
         unitName = str_split_fixed(unitName, "-", 2)[, 1])
...
vps <- vps0 %>%
  ...
  pivot_wider(names_from = c(vernacularName, stratum),
              ...
              values_fill = 0) %>%
  mutate(level = "VP", # this could be achieved with acomp()
         temp_p = rowSums(across(starts_with("pVA_"))),
         p_wg_a.Low = pVA_Watergrass_low / temp_p,
         p_wg_b.Med = pVA_Watergrass_medium / temp_p,
         p_wg_c.High = pVA_Watergrass_high / temp_p,
         p_st_a.Low = pVA_SwampTimothy_low / temp_p,
         p_st_b.Med = pVA_SwampTimothy_medium / temp_p,
         p_st_c.High = pVA_SwampTimothy_high / temp_p,
         p_sw_a.Low = pVA_Smartweed_low / temp_p,
         p_sw_b.Med = 0,
         p_sw_c.High = 0,
         p_noFood = pVA_OtherCover_NA / temp_p,
...
cps <- cps0 %>%
  ...
  pivot_wider(names_from = c(vernacularName, stratum),
              ...
              values_from = proportionVisibleArea,
              values_fill = 0) %>%
  mutate(level = "CP", # this could be achieved with acomp()
         temp_p = rowSums(across(starts_with("pVA_"))),
         p_wg_a.Low = pVA_Watergrass_low / temp_p,
         p_wg_b.Med = pVA_Watergrass_medium / temp_p,
         p_wg_c.High = pVA_Watergrass_high / temp_p,
...
qdt_st <- qdt0 %>%
  dplyr::filter(vernacularName == "SwampTimothy" & stratum != "") %>%
  group_by(GlobalID,
           LIT,
           unitName,
           subunitName,
...
qdt_wg <- qdt0 %>%
  filter(vernacularName == "Watergrass" & stratum != "") %>%
  pivot_wider(names_from = measurementType, values_from = measurementValue) %>%
  arrange(GlobalID_seedhead) %>%
  mutate(sh_emerged_wg = f2t_length / sh_length, 
         sh_mass_mg = exp(wgc[1] +
                            wgc[2] * sh_length / 10 + # divide by 10 because
                            wgc[3] * sh_emerged_wg + # model data was in cm
                            wgc[4] * f2t_length / 10) - 20) %>%
  mutate(qdt_m2 = ifelse(quadratSize == "15x15", 0.15^2,
                         ifelse(quadratSize == "5x5", 0.05^2, 0.10)),
         n_seed_head_m2 = nSeedHeads / qdt_m2) %>%
  group_by(GlobalID,
...
fpcf <- vpcp %>%
  ...
  mutate(subunitName = as.numeric(subunitName)) %>%
  left_join(su_areas) %>%
...
st_rlm3 <- rlm(log(mass_g_m2) ~ LIT_Strat,
               na.action = na.exclude,
               maxit = 40,
               data = qdt_st)
...
wg_rlm3 <- rlm(log(mass_g_m2) ~ LIT_Strat,
               na.action = na.exclude,
               maxit = 40,
               data = qdt_wg)
...
`seed_mass_fun` is a function that takes the file `p_Area`, and the models for g.m2 by subunitName and stratum ("st_rlm3" and "wg_rlm3") as arguments and yields the seed mass of st and wg per unit area of each subunit, and the area of each subunit. The resulting file can be used to calculate refuge-level yields.

```{r seed_mass_fun}

seed_mass_fun <- function(.p_Area, .qdt_st, .qdt_wg) {
  tryCatch({
    .st_rlm3 <- rlm(log(mass_g_m2) ~ LIT_Strat, # make model for st
                    na.action = na.exclude,
                    maxit = 40,
                    data = .qdt_st)
    .wg_rlm3 <- rlm(log(mass_g_m2) ~ LIT_Strat, # make model for wg
                    na.action = na.exclude,
                    maxit = 40,
                    data = .qdt_wg)
    result <- .p_Area %>% # create file to predict mass/area by stratum & LIT
      ...
      mutate(LIT_Strat = factor(ifelse(oLIT_Strat %in% .st_rlm3$xlevels[[1]],
                                       ...
                                       NA))) %>%
      mutate(st_g_m2 = unname(exp(predict(.st_rlm3, newdata = .))),
             LIT_Strat = factor(ifelse(oLIT_Strat %in% .wg_rlm3$xlevels[[1]],
                                       as.character(oLIT_Strat),
                                       NA))) %>%
      mutate(wg_g_m2 =unname(exp(predict(.wg_rlm3, newdata = .))),
      ...
      full_join(su_areas, by = c(c("LIT", "unitName", "subunitName"))) %>%
...
boot_mat <- qdt_bt %>%
  ...
  pivot_wider(id = id, names_from = subunit_ID, values_from = c(wg_g_m2, st_g_m2, tot_g_m2))
...
qdt_mass_g_m2 %>%
  dplyr::select(LIT, unitName, subunitName, wg_g_m2, st_g_m2, CI80lwr, tot_g_m2, CI80upr) %>%

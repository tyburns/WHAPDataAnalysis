...
# These estimates ignore the presence of smartweed because the dimension to mass model for smarweed is not yet available.
...
# Get lambda for swd2m
swd2m_lambda <- powerTransform(lm(mass_mg ~ log(length_mm),
                                na.action = na.exclude,
                                data = sw_dat)) %>%
  coef(round = TRUE) %>%
  unname()
...
SW_dim2mass <- lm(bcPower(mass_mg, lambda = swd2m_lambda) ~ log(length_mm),
                  data = sw_dat)
...
qdt_sw <- qdt0 %>%
  ...
  na.omit() %>%
  ungroup() %>%
  mutate(mass_g_m2 = unname(
    invBoxCox(predict(SW_dim2mass, newdata = .), swd2m_lambda) / 1000)) %>%
  mutate(stratum = dplyr::recode(stratum,
                                 `low` = "a.Low",
                                 `medium` = "b.Med",
                                 `high` = "c.High"),
...
# Note appropriate for 2021 data because KRN has wg
...
# Get lambda for sw
sw_lambda <- powerTransform(rlm(mass_g_m2 ~ LIT_Strat,
                                na.action = na.exclude,
                                maxit = 40,
                                data = qdt_sw)) %>%
  coef(round = TRUE) %>%
...
# Use robust linear model
sw_rlm3 <- rlm(bcPower(mass_g_m2, lambda = sw_lambda) ~ LIT_Strat,
               na.action = na.exclude,
               maxit = 40,
               data = qdt_sw)
  ...
  tryCatch({
    ...
    result <- .p_Area %>% # create file to predict mass/area by stratum & LIT
      ...
      mutate(wg_g_m2 = invBoxCox(unname(predict(.wg_rlm3, newdata = .)),
                                 lambda = .wg_lambda),
             LIT_Strat = factor(ifelse(oLIT_Strat %in% .sw_rlm3$xlevels[[1]],
                                       as.character(oLIT_Strat),
                                       NA))) %>%
      mutate(sw_g_m2 = invBoxCox(unname(predict(.sw_rlm3, newdata = .)),
                                 lambda = .sw_lambda),
...
qdt_mass_g_m2 <- seed_mass_fun(p_Area, qdt_st, qdt_wg, qdt_sw) %>%
  arrange(subunit_ID) %>%
  na.omit()
...
mass.su.CI <-
  ...
  na.omit() %>%
  dplyr::select(subunit_ID, wg_g_m2, st_g_m2, sw_g_m2, tot_g_m2, area_ha) %>%
...
boot_mat <- qdt_bt %>%
  ...
  unnest(cols = mass_su) %>% # put all boots estimates in one table
  na.omit() %>%
  dplyr::select(id, subunit_ID, wg_g_m2, st_g_m2, sw_g_m2, tot_g_m2) %>%
  # make matrix like df with all estimates for each boot sample in one row
  pivot_wider(id = id,
              names_from = subunit_ID,
              values_from = c(wg_g_m2, st_g_m2, sw_g_m2, tot_g_m2)) %>%
  data.matrix()

```

In this section, the bootstrap samples are used to estimate total mass of seed heads per refuge. **To Do: incorporate covariance between subunit estimates** The current calculation assumes that estimates are indepentent, which is overly optimistic.
...
mass_kg_LIT_CI %>%
  kable(digits = 0,
        format = "html",
        caption = "Seed head mass per unit area (g/m2) and 90% CI by refuge. These estimations exclude the uncertainty due to the proportion of area covered by each stratum in each unit, and the uncertainty of the model to convert seed number and seed length into mass, but they include the uncertainty due to sampling quadrats.") %>%
  kable_styling(full_width = FALSE)
...
qdt_mass_g_m2 %>%
  dplyr::select(LIT, unitName, subunitName, wg_g_m2, st_g_m2, sw_g_m2, CI80lwr, tot_g_m2, CI80upr) %>%
...
sw_qdt_means <- emmeans(sw_rlm3, "LIT_Strat", type = "response") %>%
  as.tibble() %>%
  arrange(as.character(LIT_Strat)) %>%
  dplyr::select(-df) %>%
  rename(sw_g_m2 = response)

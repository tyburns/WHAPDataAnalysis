...
st_qdt_means <- emmeans(st_lm3, "LIT_Strat", type = "response") %>%
  as.tibble() %>%
...
wg_qdt_means <- emmeans(wg_lm3, "LIT_Strat", type = "response") %>%
  as.tibble() %>%
  arrange(as.character(LIT_Strat)) %>%
  dplyr::select(-df) %>%
...
sw_qdt_means <- emmeans(sw_lm3, "LIT_Strat", type = "response") %>%
  as.tibble() %>%
  arrange(as.character(LIT_Strat)) %>%
  dplyr::select(-df) %>%

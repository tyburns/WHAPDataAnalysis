...
SW_dim2mass <- lm(bcPower(mass_mg, lambda = 0.33) ~ log(vol_mm3) + length_mm,
            data = sw_dat)
...
qdt_sw <- qdt0 %>%
  ...
  na.omit() %>%
  ungroup() %>%
  mutate(mass_g_m2 = unname((0.33 * predict(SW_dim2mass, newdata = .) + 1)^(1/0.33)) / 1000) %>%
  mutate(stratum = dplyr::recode(stratum,
                                 `low` = "a.Low",
                                 `medium` = "b.Med",
                                 `high` = "c.High"),
